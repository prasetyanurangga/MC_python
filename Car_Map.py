...
class CarMap:
  def __init__(self):
    ...
    self.in_grey = [160] * round((len(self.in_map) / 2))
    self.out_grey = [160] * round((len(self.out_map) / 2))
    self.blue1 = [0, 39, 102,1]*round((len(self.score_lines)/2))
    self.blue2 = [0, 97, 255, 1] * (2)
    self.in_batch.add(round(len(self.in_map) / 2), pyglet.gl.GL_LINE_LOOP, None, ('v2i', self.in_map),
                      ('c4B', self.in_grey * 4))
    self.out_batch.add(round(len(self.out_map) / 2), pyglet.gl.GL_LINE_LOOP, None, ('v2i', self.out_map),
                       ('c4B', self.out_grey * 4))
